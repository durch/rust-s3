...
impl GetAndConvertHeaders for http_types::Headers {
    fn get_and_convert<T: FromStr>(&self, header: &str) -> Option<T> {
        self.get(header)?.to_string().parse::<T>().ok()
    }
    fn get_string(&self, header: &str) -> Option<String> {
        Some(self.get(header)?.to_string().to_owned())
    }
}

#[cfg(feature = "with-smol")]
impl From<&http_types::Headers> for HeadObjectResult {
    fn from(headers: &http_types::Headers) -> Self {
